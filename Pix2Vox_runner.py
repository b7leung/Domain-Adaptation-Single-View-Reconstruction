...
from core.Pix2Vox.train import train_net
...
def get_args_from_command_line():
    ...
    parser.add_argument('--verbose', dest='verbose', help='Print more details while running', action='store_true',
                        default=cfg.PREFERENCES.VERBOSE)

    # training/testing arguments
    parser.add_argument('--test_dataset', dest='test_dataset',
                        help='What dataset to test on. Valid options are ShapeNet and ShapeNetPlaces.',
                        default="ShapeNet", type=str)
    parser.add_argument('--train_source_dataset', dest='train_source_dataset',
                        help='Source dataset to train on. Valid options are ShapeNet and ShapeNetPlaces.',
                        default="ShapeNet", type=str)
    ...
    parser.add_argument('--use_train_set', dest='use_train_set', help='Use training set when testing',
                        action='store_true', default=False)
    parser.add_argument('--batch_size', dest='batch_size', help='Set batch size during training.',
                        default=cfg.CONST.BATCH_SIZE, type=int)
    parser.add_argument('--epoch', dest='epoch', help='num. of epoches. Only has effect for training',
                        default=cfg.TRAIN.NUM_EPOCHES, type=int)
    parser.add_argument('--save_num', dest='save_num',
                        help='During testing only: Save n reconstructions per class. If -1, save all.',
                        default=cfg.TEST.SAVE_NUM, type=int)
    parser.add_argument('--classes', dest="classes_to_use", nargs='+', help='Classes to use; use shapenet convention names, they will be converted as needed.\
                        Shapenet classes are aeroplane, bench, cabinet, car, chair, display, lamp, speaker, rifle, sofa, table, telephone, watercraft. ',
                        default=None)
    parser.add_argument('--num_views', dest='num_views', help='number of views to use',
                        default=cfg.CONST.N_VIEWS_RENDERING, type=int)
    ...
    # CORAL arguments
    parser.add_argument('--coral_lam', dest='coral_lam', help='lambda for CORAL loss',
                        default=cfg.TRAIN.DA.CORAL_LAMBDA, type=float)

    # DANN arguments
    parser.add_argument('--DANN_lam', dest='dann_lam', help='lambda for DANN', default=cfg.TRAIN.DA.DANN_LAMBDA,
                        type=float)

    # Voxel Classification arguments
    parser.add_argument('--voxel_lam', dest='voxel_lam',
                        help='lambda for VoxelCls loss. -1 is adaptive linear, -2 is adaptive DANN',
                        default=cfg.TRAIN.VOXEL_CLASSIFIER_LAMBDA, type=int)
